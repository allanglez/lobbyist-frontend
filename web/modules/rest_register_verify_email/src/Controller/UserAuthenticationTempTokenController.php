 ...
 */
class UserAuthenticationTempTokenController extends ControllerBase implements ContainerInjectionInterface {
  ...
  public function login(Request $request) {
    ...
    if ($uid = $this->userAuth->authenticate($credentials['name'], $credentials['pass'])) {
      ...
      if (is_array($user) && count($user) == 1) {
        ...
        $temp_token = $tempstore->getIfOwner('temp_token');
        if (!empty($temp_token)) {
          // Check that the dude supplied the right temp pass.
          if ($credentials['pass'] === $temp_token) {
            // Yep we got a match.
            $this->flood->clear('user.http_login', $this->getLoginFloodIdentifier($request, $credentials['name']));
            ...
            $logout_path = ltrim($logout_route->getPath(), '/');
            $response_data['logout_token'] = $this->csrfToken->get($logout_path);
            $response_data['temp_token'] = 'A Temp password has ben used please update your password';
            $encoded_response_data = $this->serializer->encode($response_data, $format);

            // Delete temp pass as they have logged in ? as it will expire.
            // $tempstore->deleteIfOwner('temp_token'); // nah leave it for now.
            return new Response($encoded_response_data);
          }
        }
      }
